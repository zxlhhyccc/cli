...
const logHandler = (level, prefix, ...args) => {
  // TODO: open append only file and write to it per event and delete on success
  logRecord.push({
    id: logId++,
    level,
    prefix,
...
process.on('exit', code => {
  ...
  // npm.log.disableProgress() // TODO: proggy
...
const exitHandler = (err) => {
  ...
  if (typeof npm.updateNotification === 'string') {
    // TODO: this is the only (?) place where loglevel is get/set outside of init
    // should this be refactored to just always be displayed by the view layer?
    const { level } = npm.log
