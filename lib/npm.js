...
const npm = module.exports = new class extends EventEmitter {
  ...
  async [_load] () {
    ...
    this.display.setConfig({
      ...
      unicode: config.get('unicode')
    })
    process.emit('timeEnd', 'npm:load:setupLog')

    // Resume logs now that we know what loglevel we are displaying
    log.resume()

    process.env.COLOR = this.color ? '1' : '0'

    process.emit('time', 'npm:load:cleanupLog')
    cleanUpLogFiles(this.cache, this.config.get('logs-max'))
  ...
  output (...msg) {
    this.display.output(...msg) 
  }

  outputError (...msg) {
    this.display.outputError(...msg)
  }
}()
