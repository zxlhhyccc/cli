...
const processOnTimeEndHandler = (name) => {
  if (timers.has(name)) {
    ...
    procLog.timing(name, `Completed in ${ms}ms`)
    timings[name] = ms
    timers.delete(name)
  } else
    procLog.silly('timing', "Tried to end timer that doesn't exist:", name)
}
...
const LogListener = require('./utils/log-listener')
const patchLogWarn = require('./utils/patch-log.js')
...
const npm = module.exports = new class extends EventEmitter {
    ...
    if (!warnedNonDashArg) {
      args.filter(arg => /^[\u2010-\u2015\u2212\uFE58\uFE63\uFF0D]/.test(arg))
        .forEach(arg => {
          ...
          procLog.error('arg', 'Argument starts with non-ascii dash, this is probably invalid:', arg)
        })
    }
      ...
      this.loadPromise = new Promise((resolve, reject) => {
        this[_load]().catch(er => er).then((er) => {
          ...
          if (!er && this.config.get('force'))
            procLog.warn('using --force', 'Recommended protections disabled.')
  ...
  async [_load] () {
    ...
    if (node && node.toUpperCase() !== process.execPath.toUpperCase()) {
      procLog.verbose('node symlink', node)
    ...
    this.logListener.setConfig(this.config)
    // patchLog()
