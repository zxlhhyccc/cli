...
class View extends BaseCommand {
  ...
  async view (args) {
    ...
    if (!this.npm.config.get('json') && wholePackument) {
      ...
      // log.disableProgress() // TODO: proggy
  ...
  prettyView (packument, manifest) {
    ...
    if (info.repo || info.site)
      info.site && this.npm.output(color.cyan(info.site))
    ...
    if (info.keywords.length) {
      this.npm.output('')
      this.npm.output('keywords:', info.keywords.join(', '))
    }

    if (info.bins.length) {
      this.npm.output('')
      this.npm.output('bin:', info.bins.join(', '))
    }
